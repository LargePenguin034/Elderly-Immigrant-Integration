...
export default function HomeScreen() {
  ...
  // Translate the input speech whenever it changes
  useEffect(() => {
    const translateText = async () => {
      if (inputSpeech) {
        console.log("Translating:", inputSpeech);
        const translatedText = await audioUtils.translateText(inputSpeech, language);  // Use utility function for translation
        console.log("Translation result:", translatedText);
        setTranslation(translatedText);  // Update state with translated text
      }
    };
  ...
  const handlePress = () => {
    if (recording) {
      ...
      audioUtils.stopStreaming(recording);  // Stop recording if it's currently active
      ...
      setRecording(true);  // Mark recording as active
    }
  };
